...
const isAuth = require('../middleware/is-auth'); // brings in our 'us-auth.js' file from the middleware folder.
...
router.get('/posts', isAuth, feedController.getPosts); // this is the route to our newsfeed posts. When a request reaches this it checks the isAuth.js in our middleware folder then goes to the controllers/feed.js and executes the 'getPosts' logic there.

// POST /feed/post - '/feed/post' is the path that handles the POST request.
router.post('/post',
     isAuth, // checks for the authorization token
...
router.get('/post/:postId', isAuth, feedController.getPost); // this route will fetch our post with the dynamic (as indicated by the colon (:)) ':postId'. It uses the FeedController getPost function.
...
router.put('/post/:postId',  // The 'put' method won't work with a browser form but with JavaScript's asynchonous requests, we can use it. 'Put' will create or overwrite a resource on the server.
    isAuth, // checks for authorization
    [ // this object [] contains our server-side validation using express-validator and calling the 'body' const defined above to check the body.
    body('title')
    .trim()
    .isLength({ min: 5 }),
...
router.delete('/post/:postId', isAuth, feedController.deletePost); // Route uses the 'delete' method to delete posts. Routes always have an associated controller action.
